...
export async function run() {
    ...
    const gearMap = Array.from(Array(lines[0].length + 2), () => Array.from(Array(lines.length + 2), () => 0));
    ...
    for (let row = 0; row < lines.length; row++) {
        for (let col = 0; col < lines[row].length; col++) {
            if (lines[row][col] === '*') {
                ...
                maskMap[row][col + 2] = 7;
                maskMap[row + 1][col] = 6;
                maskMap[row + 1][col + 1] = 5;
                maskMap[row + 1][col + 2] = 4;
                maskMap[row + 2][col] = 3;
                maskMap[row + 2][col + 1] = 2;
    ...
    lines.forEach((line, index) => {
        const check = (i: number) => {
            for (let col = i - len; col < i; col++) {
                if (maskMap[index + 1][col + 1] > 0) {
                    const gearRow = index + Math.floor((maskMap[index + 1][col + 1] - 1) / 3);
                    const gearCol = col + ((maskMap[index + 1][col + 1] - 1) % 3);

                    if (gearMap[gearRow][gearCol] > 0) {
                        total += num * gearMap[gearRow][gearCol];
                    } else {
                        gearMap[gearRow][gearCol] = num;
                    }
